...
public:
	...
	RejectReasonDesc()
	{
		...
		m_newOrdergrammarDesc[7]  = "7.账户不存在;";
		...
		m_riskRuleDesc[33]= "33.订单申报金额控制;";
		m_riskRuleDesc[34]= "34.交易金额控制;";
		m_riskRuleDesc[35]= "35.买入额度上限;";
		m_riskRuleDesc[36]= "36.同一合约品种权利仓持仓数量上限;";
		m_riskRuleDesc[37]= "37.同一合约品种认购期权权利仓持仓数量上限;";
		m_riskRuleDesc[38]= "38.同一合约品种认沽期权权利仓持仓数量上限;";
		m_riskRuleDesc[39]= "39.同一合约品种义务仓持仓数量上限;";
		m_riskRuleDesc[40]= "40.同一合约品种认购期权义务仓持仓数量上限;";
		m_riskRuleDesc[41]= "41.同一合约品种认沽期权义务仓持仓数量上限;";
		m_riskRuleDesc[42]= "42.同一合约品种总持仓数量上限;";
		m_riskRuleDesc[43]= "43.同一合约品种多头持仓数量上限;";
		m_riskRuleDesc[44]= "44.同一合约品种空头持仓数量上限;";
		...
		m_riskRuleDesc[50] = "50.期权卖出开仓限额";
		m_riskRuleDesc[51] = "51.期权总成交数量";
		m_riskRuleDesc[52] = "52.期权订单手数限制";
	...
	string GetOrderRejectDesc(int grammarResult, unsigned char* riskResult)
	{
		...
		if (0 != grammarResult)
		{
			for (int bitIdx = 1; bitIdx <= 24; ++bitIdx)
			{
		...
		int ACC_RISK_LEN = 16;
		for (int ruleIdx = 1; ruleIdx <= 54; ++ruleIdx)
		{
			int chBitLen = 8;
